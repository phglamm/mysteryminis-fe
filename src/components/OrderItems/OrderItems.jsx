...
const OrderItems = ({selectedCategory, setViewDetails}) => {
  ...
  return (
    ...
    <motion.div 
            ...
            >
         <AnimatePresence>
                {(ViewDetails && selectedOrder ? [selectedOrder] : filteredOrders).map((order, index) => (
                    <motion.div
                        key={index}
                    ...
                    <motion.div
                            ...
                            <motion.div
                                className={`p-[2%] flex flex-col border-y-1 border-dashed ${ViewDetails ? "" : "hidden"}`}
                                initial={{ opacity: 0, y: -50, height: 0 }} 
                                animate={ViewDetails ? { opacity: 1, y: 0, height: "fit-content" } : { opacity: 0 }}    
                                exit={{ opacity: 0, y: -50, height: 0 }} 
                                transition={{ duration: 1, type: "spring", damping: 10 }}
                            >
                                ...
                                <span className='flex flex-row '>
                                    <span className='flex flex-col w-[30%] '>
                                        ...
                                        <span className='text-[0.9vw] text-gray-400'>{user.phone} </span>
                                    </span>
                                    <span className='text-[1.2vw] w-[70%] text-gray-400'>
                                        {order.address?.addressDetail ?? ''}  {order.address?.district ?? ''} {order.address?.province ?? ''}  {order.address?.ward ?? ''}                       
                                    </span>
                                </span>
                            </motion.div>
                                    ...
                                    >
                                        <div className='flex gap-1 w-2/4'>
                                            <div className='w-1/4 bg-pink-200'>
                                                <img src={item.imageUrl} alt={item.boxName} className='w-full h-full object-cover' />
                                            </div>
                                            <div className='w-3/4 flex flex-col'>
                                                <span className='font-bold text-xl'>{item.boxName}</span>
                                                <span className='text-sm text-gray-500'>{item.boxOptionName}</span>
                        ...
                        <div className=' gap-4 px-8 p-2 bg-white h-fit border-dashed border-t-1'>
                            <div className='flex flex-col text-end text-2xl w-full gap-4 py-4'>
                                ...
                                <div className='text-sm text-gray-500'>Payment Method: {order.paymentMethod} </div>
                            </div>

                                <div className='flex justify-end gap-4'>
                                    {order.orderStatusDetailsSimple?.slice(-1)[0]?.statusName === 'Pending' && order.paymentMethod === 'COD' ? (
                                                <motion.button
                                                    className='border-1 px-3 py-1 w-[25%] text-[0.9vw] rounded-md font-bold'
                                                    initial={{ backgroundColor: loadingCancel ? 'gray' : '#ef4444',  color:'white', border: '1px solid #f3f4f6' }}
                                                    whileHover={{ backgroundColor: loadingCancel ? 'gray' : '#ef4444', color: 'white', border: '1px solid white', scale: 1.1 }}
                                                    whileTap={{ scale: 0.9 }}
                                                    onClick={() => cancelOrders(order.orderId)}
                                                    disabled={loadingCancel}
                                                >
                                                    {loadingCancel ?  <><LoadingOutlined /> Cancelling...</> : 'Cancel Order'}
                                                </motion.button>
                                    ) : 

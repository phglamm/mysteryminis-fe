...
console.log(isEditing);
  ...
  const fetchAddress = async () => {
    try {
      ...
      const fetchedAddresses = Array.isArray(response.data) ? response.data : [response.data];
    ...
    <div>
      {!isEditing ? (
        <div className="h-[30vw] pb-28 overflow-y-auto">
          {addresses.length === 0 ? (
            <p className='w-full h-full flex justify-center items-center'><Spin size="large"/></p>
          ) : (
              ...
              <motion.div key={address.addressId} className="p-4 border-b">
                <span className="flex flex-row">
                  <span className="flex flex-col w-[30%]">
                    ...
                    <span className="text-[0.9vw] text-gray-400">{address.phoneNumber}</span>
                  </span>
                  ...
                  <span 
                    onClick={() => { setIsEditing(true); setSelectedAddress(address); }} 

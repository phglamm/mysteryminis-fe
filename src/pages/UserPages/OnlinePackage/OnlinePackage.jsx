...
const OnlinePackage = () => {
    ...
    const groupedByBrand = boxes.reduce((acc, box) => {
        if (!acc[box.Brand]) {
            acc[box.Brand] = [];
        }
                ...
                <div key={brand} className="mb-6 w-full text-center px-[20%]">
                    ...
                    <div className="flex flex-wrap gap-10 justify-center">
                        {groupedByBrand[brand].map((box) => (
                            <Badge.Ribbon key={box.onlineSerieBoxId} text={`Box ID: ${box.onlineSerieBoxId}`} color={box.OnlineSerieBox.IsSecretOpen ? "lime" : "gold"}>
                                <Card
